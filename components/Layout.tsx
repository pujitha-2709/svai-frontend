...
export const Layout: React.FC<LayoutProps> = ({ currentView, onNavigate, children, user, onLogout, unreadCount = 0 }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors duration-200">
      ...
      {/* Sidebar */}
      <aside className={`hidden md:flex md:flex-shrink-0 w-64 ${theme === 'dark' ? 'bg-slate-900' : 'bg-slate-100'} border-r ${theme === 'dark' ? 'border-slate-800' : 'border-slate-300'} flex-col`}>
        <div className="p-6 border-b border-slate-800 dark:border-slate-800">
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => onNavigate(View.DASHBOARD)}>
            ...
            <h1 className="text-xl font-bold tracking-tight text-white dark:text-white">SkillVouch AI</h1>
          </div>
        </div>

        <nav className="flex-1 p-4 overflow-y-auto">
          <div className="mb-2 px-4 text-xs font-semibold text-slate-500 dark:text-slate-500 uppercase tracking-wider">Platform</div>
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
           ...
           <button
             onClick={toggleTheme}
             className="w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-slate-400 hover:bg-slate-800/50 hover:text-slate-200 transition-colors mb-1"
           >
             {theme === 'dark' ? (
           ...
           <div className="mt-4 px-4 flex items-center space-x-3">
              <img src={user.avatar} alt="User" className="w-8 h-8 rounded-full border border-slate-700 dark:border-slate-600" />
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden md:ml-0">
        {/* Mobile Top Bar */}
        <div className={`md:hidden bg-slate-900 ${theme === 'dark' ? 'border-slate-800' : 'border-slate-300'} px-4 py-3 flex items-center justify-between`}>
          <div className="flex items-center space-x-3">
            <Logo className="w-6 h-6" />
            <h1 className="text-lg font-bold text-white dark:text-white">SkillVouch AI</h1>
          </div>
        </div>
      ...
      <div className={`md:hidden fixed bottom-0 left-0 right-0 bg-slate-900 ${theme === 'dark' ? 'border-t border-slate-800' : 'border-t border-slate-300'} flex justify-around p-2 z-40 transition-colors duration-200`}>
        <button 
          onClick={() => onNavigate(View.DASHBOARD)} 
          className={`flex flex-col items-center p-2 rounded-lg transition-colors ${
            currentView === View.DASHBOARD ? 'text-indigo-400' : 'text-slate-400 dark:text-slate-200'
          }`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center p-2 rounded-lg transition-colors ${
            currentView === View.MY_SKILLS ? 'text-indigo-400' : 'text-slate-400 dark:text-slate-200'
          }`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center p-2 rounded-lg transition-colors ${
            currentView === View.FIND_PEERS ? 'text-indigo-400' : 'text-slate-400 dark:text-slate-200'
          }`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center p-2 rounded-lg transition-colors relative ${
            currentView === View.MESSAGES ? 'text-indigo-400' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
